...
const main = async () => {
  ...
  for (const j of seen.values()) {
    ...
    if (existing.includes(link)) { counts.dupe++; continue; }
  ...
  console.log(`dropped: ${counts.title} title, ${counts.hq} HQ, ${counts.geo} geo, ${counts.dupe} already known`);
  console.log(`\nKEEPING ${kept.length}:\n`);
  for (const k of kept) {
    console.log(`  ${k.sub ? '★ SUBSIDIARY' : '  amazon    '} ${k.company} | ${k.title.slice(0, 52)} | ${k.loc}`);
  }
  ...
  await appendFile(
    PIPELINE,
    '\n' + kept.map((k) => `- [ ] ${k.url} | ${k.company} | ${k.title} | source: amazon-jobs | ${today}`).join('\n') + '\n'
  );
  await appendFile(
    HISTORY,
    kept.map((k) => `${k.url}\t${today}\tamazon-jobs\t${k.title}\t${k.company}\tadded\n`).join('')
  );
  console.log(`\nappended ${kept.length} to pipeline.md`);
};
