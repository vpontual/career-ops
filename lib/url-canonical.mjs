...
export const TRACKING_PARAMS = new Set([
  ...
  "ref", "src", "source",
  // Per-EMAIL and per-RECIPIENT identifiers. These change on every send, so the
  // same posting arrives with a different URL each time and dedup never fires.
  // Measured on Jobot: 49 distinct postings were written to pipeline.md as 168
  // rows - 119 redundant - because every URL carried ?eid=<email>&uid=<user>.
  // `jid` is NOT stripped: it is the job id and is what actually identifies the
  // posting once the email noise is gone.
  "eid", "uid", "mkt_tok", "_bhlid", "igshid", "epik", "s_kwcid", "vero_id",

  // Noise picked up by FOLLOWING a redirect, added 2026-09-02 with lib/apply-url.mjs.
  // Indeed's `job_url_direct` reaches the employer through short links and
  // trackers (grnh.se, *.contacthr.com, jsv3.recruitics.com), and each stamps
  // the destination with a per-request token. Without stripping `gh_src` the
  // same Greenhouse job arriving via two Indeed rows canonicalizes to two URLs,
  // which is precisely the dedup failure this file was written to end.
  // Measured before adding: 0 existing rows in pipeline.md, scan-history.tsv or
  // jds/ carry any of these, so no stored URL changes meaning.
  "gh_src", "rx_viewer", "indeed-apply-token", "iis", "iisn", "feedid",
]);
