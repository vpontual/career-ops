...
export function detectTrack(jd) {
  ...
  if (NOW_TITLE.test(title)) return 'now';
...
export function titlePassesForTrack(track, title) {
  ...
  if (track === 'now') return !NOW_EXCLUDE.test(t);
  return false;
}
...
export function trackFacts(track, jd) {
  ...
  if (track === 'now') {
    return {
      ...
      seniorEnough: /\b(senior|staff|lead|principal|director|head|group|vp|chief)\b/i.test(title),
      // VP is fluent in English, Portuguese, Spanish and French. A role in those
      // markets is one where he competes against far fewer people, so it counts
      // FOR the role rather than being a complication.
      languageEdge: /\b(brazil|brasil|portuguese|português|latam|latin america|s[ãa]o paulo|rio de janeiro|mexico|m[ée]xico|spain|espa[nñ]a|spanish|france|french|fran[çc]ais|portugal|lisbon|colombia|argentina|chile)\b/i.test(hay),
 ...
 * search. VP set this one deliberately loose on 2026-08-04: no geography gate at
 * all, no comp floor, no archetype. Another country and another currency are the
 * point, not a problem. The only things that still disqualify are work he cannot
 * do (a live-coding screen) and hours-for-minimum-wage work, which is excluded by
 * title before scoring ever runs.
 */
export function scoreNow(f) {
  if (f.technicalScreen) return 1;

  let score = 3;
  if (f.fastStart) score += 1;      // fractional / interim / contract / consulting
  if (f.seniorEnough) score += 1;   // pays like fifteen years, not like an associate
  if (f.languageEdge) score += 1;   // Portuguese, Spanish or French market
  if (f.remote) score += 1;
  if (f.compLow != null && f.compLow < 40000) score -= 1;
