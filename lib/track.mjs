...
export function titlePassesForTrack(track, title) {
  ...
  if (track === 'now') return !NOW_EXCLUDE.test(t) && !NOW_TOOLING.test(t) && !NOW_WRONG_FIELD.test(t);
  return false;
}
...
export function scoreNow(f) {
  ...
  if (f.technicalScreen) return 1;   // cannot pass a live-coding screen
  if (f.hardCredential) return 1;    // a clearance or licence he does not hold
  if (f.wrongField) return 1;        // a specialism he has never practised
