...
/**
 * The tracks that exist, in the order VP reads them, with the one-line
 * description the review UI prints under the tab bar.
 *
 * ⚠ ONE DEFINITION. data/review-queue.json carries a `tracks` legend that was
 * written by hand once and then never again: on 2026-09-08 it still described
 * `govtech` and `venture` — retired labels with zero pending cards — and named
 * neither `civic` nor `now`, which between them were 83 of the 287 cards on the
 * board. So the two biggest non-PM tabs rendered with no description at all
 * while two dead ones were documented. enqueue-review writes the legend from
 * here on every run, so it cannot go stale again, and detectTrack can only ever
 * return one of these keys.
 *
 * Keys retired from detectTrack stay OUT of this object. Old cards carrying
 * them are still rendered — the UI falls back to the raw key — they simply stop
 * being advertised as somewhere new work can land.
 */
export const TRACK_LABELS = Object.freeze({
  pm: 'PM / PMM — the standard search',
  civic: 'City of New York — public sector, where an exam is a door rather than a wall',
  now: 'Get Hired Now — ranked on time-to-income, not on NYC fit',
  nonprofit: 'Nonprofit / charity — product skills, community impact',
  teaching: 'Teaching — college, community college, or NYC public school',
});
