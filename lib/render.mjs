...
function cvMarkdownToHtml(cvMd) {
  // ⚠ STRIP HTML COMMENTS BEFORE ANYTHING ELSE.
  //
  // The next line escapes < and >, so an un-stripped comment does not stay a
  // comment - it becomes visible body copy in the submitted PDF. cv.md carries a
  // truth-guard comment reading "NOT a founding PM at Reco; did NOT define the
  // technical roadmap or API surface", and variant-scoping tags on individual
  // bullets. Rendered, an employer would read all of it as the candidate's own
  // text. Verified 2026-08-10 by rendering cv.md through this function.
  //
  // Today this cannot fire because stage-applications falls back to cv.md only
  // when cv-<variant>.md is missing and all six exist - so a single typo in a
  // variant name is the entire safety margin. That is not a margin.
  const lines = String(cvMd ?? "").replace(/<!--[\s\S]*?-->/g, "").split("\n");
  const html = [];
  let inList = false;
