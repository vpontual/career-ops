 ...
 *
 * WHY canonKey ALONE IS NOT ENOUGH HERE. nightly-report.mjs asks a different
 * question from the staging gate above: not "should I rebuild this pack?" but
 * "did VP actually submit this approved card?". Both directions of error are
 * expensive — a miss nags him about an application already sent, a false match
 * hides the one thing the report exists to surface — and on 2026-09-02 both
 * were live at once:
 *
 *   • Matching on COMPANY NAME (what the report did) hid GitLab | Senior
 *     Product Manager, Growth — approved 09-01, never submitted — because two
 *     other GitLab roles were already on the tracker.
 *   • Matching on canonKey alone would have nagged about Anthropic | Product
 *     Manager, Claude Code, submitted 08-05 and recorded under the posting's
 *     fuller title "Product Manager, Claude Code / Claude Tag". Same
 *     application, different canonKey.
 *
 * So a card counts as applied if ANY of three identities match: the posting URL
 * (written into Notes by ui/app/api/status/route.ts, and already the join
 * ui/lib/pipeline.ts uses), the output/<slug>/ path in the PDF column (how the
 * hand-backfilled 2026-08-10 rows identify themselves), or canonKey as the
 * fallback for rows carrying neither.
 *
 * ⚠ CLOSED_STATUSES still governs. `evaluated` means "on the tracker, not yet
 * sent" — exactly what the report should be shouting about — so it must never
 * count as applied here either.
 */

const ROW_URL_RE = /https?:\/\/[^\s|)]+/g;
const ROW_SLUG_RE = /output\/([A-Za-z0-9][A-Za-z0-9-]*)\//g;

const safeCanonicalUrl = (u) => {
  try { return canonicalizeUrl(String(u ?? '').trim()) || ''; } catch { return ''; }
};

/** Parse applications.md text → the three identity sets of its CLOSED rows. */
export function parseAppliedIdentities(text) {
  const keys = new Set(), urls = new Set(), slugs = new Set();
  for (const line of String(text ?? '').replace(/\r/g, '').split('\n')) {
    if (!line.startsWith('|')) continue;
    const parts = line.split('|').map(s => s.trim());
    if (parts.length < 9) continue;
    ...
    const company = parts[3], role = parts[4];
    if (!company || !role) continue;
    if (!CLOSED_STATUSES.has(normalizeStatus(parts[6]))) continue;

    keys.add(canonKey(company, role));
    for (const m of line.matchAll(ROW_URL_RE)) {
      const u = safeCanonicalUrl(m[0]);
      if (u) urls.add(u);
    }
    for (const m of line.matchAll(ROW_SLUG_RE)) slugs.add(m[1].toLowerCase());
  }
  return { keys, urls, slugs };
}

/** Load data/applications.md (absent → empty sets = nothing counted applied). */
export async function loadAppliedIdentities(root) {
  try {
    return parseAppliedIdentities(await readFile(path.join(root, 'data', 'applications.md'), 'utf-8'));
  } catch {
    return { keys: new Set(), urls: new Set(), slugs: new Set() };
  }
}

/** Has this queue card already been submitted? URL, then slug, then canonKey. */
export function isApplied(card, identities) {
  if (!card || !identities) return false;
  const { keys, urls, slugs } = identities;

  const url = safeCanonicalUrl(card.applyUrl || card.sourceUrl);
  if (url && urls?.has(url)) return true;

  const slug = String(card.slug ?? '').trim().toLowerCase();
  if (slug && slugs?.has(slug)) return true;

  return Boolean(keys?.has(canonKey(card.company, card.role)));
}
