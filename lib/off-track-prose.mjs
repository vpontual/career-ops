...
/**
 * Punctuation the model leaves behind, tidied for display on EVERY track.
 *
 * Two artifacts, both visible on live cards before this existed:
 *   - `.,` — the model returns redFlags as a comma-joined list and the items
 *     already end in full stops, so the board rendered "...in-person
 *     interview.,Role requires working from the office two days per week."
 *   - `..` — enqueue-review writes `SCORER: ${verdict}.` and the verdict
 *     usually ends in one already.
 *
 * ⚠ Formatting only. It never removes a claim, so it is safe on pm.
 */
...
/**
 * What a card should SHOW for one piece of the scorer's prose.
 *
 * ⚠ THE ONE ENTRY POINT FOR DISPLAY. There are two rendering paths and I fixed
 * only one of them the first time: review cards read the queue's `notes`, while
 * the home-page views (Shortlist, Ready to apply, Ranked, All roles) read
 * `verdict` and `redFlags` STRAIGHT OFF data/lead-scores.json. So "Ready to
 * apply" still showed "Role is a Policy Analyst position, not a Product or
 * Product Marketing role" on a civic role after the review queue had been
 * cleaned. Anything that renders this prose goes through here.
 *
 * Punctuation is tidied on every track; category errors are removed only on the
 * four tracks with their own rubric.
 */
export function displayProse(track, text) {
  return tidyProse(stripOffTrackClaims(track, tidyProse(text)));
}
...
export function cleanNotesForTrack(track, notes) {
  ...
  // ⚠ RUNS ON EVERY TRACK, INCLUDING pm. It used to return a pm card
  // byte-identical, which left `SCORER: ${verdict}.` rendering as "...in
  // consumer-facing AI products.. Comp floor seen: $240,000." on every PM card
  // in the queue — the verdict already ends in a full stop. displayProse only
  // REMOVES a claim on the four rubric tracks; on pm it tidies punctuation and
  ...
  for (const marker of ['SCORER: ', 'RED FLAGS: ']) {
    ...
    out = replaceSegment(out, marker, displayProse(track, out.slice(bodyStart, end)));
  }
  return tidy(out);
}
