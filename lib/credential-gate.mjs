...
// ⚠ Government-contractor boilerplate is the single biggest false-positive risk
// for `security clearance`, and it is NOT rare - it is the standard paragraph.
// Found 2026-08-11 on two live Nava PBC roles, both hard-gated to 1:
//   "May be subject to a government background check or security clearance,
//    depending on the contract"
// That is a clearance the employer may INITIATE later, not one VP must already
// hold, and it reads identically at Ad Hoc, Nava and most civic-tech vendors -
// exactly the employers Track B and Track E exist to surface. `may be required`
// and `depending on the role` were already here; `may be subject to` and
// `depending on the contract` are the phrasings those postings actually use.
const SOFT_WORDS = /\b(?:preferred|preference|nice to have|nice[- ]to[- ]have|a (?:strong |big |huge |major )?plus|plus\b|bonus|desirable|ideally|would be great|helpful|advantageous|familiarity|exposure to|we'?d love|not required|no(?:t)? necessary|willing(?:ness)? to obtain|abilit(?:y|ies) to (?:\w+ )?obtain|able to (?:\w+ )?obtain|eligib\w+ to (?:\w+ )?obtain|we (?:will )?(?:sponsor|support|pay for)|or (?:the )?(?:field[- ]earned )?equivalent|equivalent (?:experience|work experience|practical experience)|or comparable|may be required|may be subject to|subject to (?:an?\\s+)?(?:government\\s+)?(?:background\\s+check|investigation)|if (?:you are|applicable|required)|depending on the (?:position|role|contract|client|project|assignment|program)|some (?:roles|positions))\b/i;
