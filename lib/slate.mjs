 ...
 *
 *   1. tier            the rubric's whole output; everything below is a
 *                      tie-break between roles the rubric rated equally.
 *   2. whale           VP, 2026-08-05: "unless its a whale like anthropic" -
 *                      a company he would drop everything for. A whale 5 beats
 *                      a non-whale 5 even when the whale is older.
 *   3. live recency    the survival curve in lib/freshness.mjs: being late
 *                      costs more than anything below can buy back.
...
export function packReadiness(item, packs) {
  if (!packs || typeof packs !== 'object') return { checked: false, answers: false, coverOk: false, level: 0 };
  ...
  return { checked: true, answers, coverOk, level: (answers ? 1 : 0) + (coverOk ? 1 : 0) };
}
...
function whyFor(f, slot, today) {
  ...
  if (!f.ready.checked) out.push('pack not checked');
    ...
    out.push(f.ready.answers ? 'answers drafted' : 'no answers.md yet');
