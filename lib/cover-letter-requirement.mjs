...
const JD_REQUIRED = [
  ...
  // A direct instruction to send one.
  //
  // ⚠ `include` USED TO BE EXCLUDED HERE and is now back, deliberately. The
  // original reason was real — "Current Employees please include your ERN on
  // your cover letter and resume" is an instruction about the ERN, not about
  // sending a letter — but that case is now caught upstream by the
  // `current employees` entry in JD_NOT_A_REQUIREMENT, which is the precise
  // guard for it. Dropping the verb was the blunt fix for a problem that now
  // has a sharp one.
  //
  // The cost of leaving it out: every NYC Public Schools posting closes with
  // "Please include a resume and cover letter with your application", and all 8
  // in the corpus resolved to `unknown`. Three of them were staged as packs on
  // 2026-08-14 carrying cover-letter-skipped.md — i.e. VP's standing rule ("if
  // it is optional, do not submit one") was applied to postings that require
  // one, which is the expensive direction to be wrong in.
  //
  // Measured across all 2,330 JDs before the change: 19 -> 27 required, and
  // every one of the 8 gained is the identical unambiguous NYCPS sentence. No
  // false positives, and the ERN posting still does not match.
  /\bplease\s+(?:submit|upload|attach|provide|include|enclose)\b[^.\n]{0,90}\bcover letter\b/i,
];
